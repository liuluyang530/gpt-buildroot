...
mkfsExt4(){
	...
	do
		...
		if [ $partSize -ne $fsSize ];then
			print_log "mkfsExt4 $line, because partSize $partSize not equal fsSize $fsSize."
			mkfs.ext4 -F $line
			sync
		elif [ $confForceFormat -ne 0 ]; then
		...
		if [ $? -ne 0 ]; then
			...
			print_log "[SUCCESS]***** [$line] ***** disk can mount with ext4, do not exec format!"
			umount -Avf $line
		fi
		sleep 1
	done
